...
| GET    | `/admin`                       | Cloudflare Access | Admin page with two tabs: Images (upload) + Editor (markdown 写作台); UI is English-only |
...
**AI proxy**: model prefix routes to the provider's OpenAI-compat endpoint (`gpt-*`→OpenAI, `claude-*`→Anthropic, `gemini-*`→Google, `grok-*`→xAI, `deepseek-*`→DeepSeek, `cf-{author}/{model}`→Workers AI via its OpenAI-compat REST route + `CF_AI_TOKEN` secret, rewritten to `@cf/{author}/{model}`; all `Authorization: Bearer`), request body untouched. `CF_ACCOUNT_ID`/`AIG_GATEWAY` vars optionally route the non-`cf` providers through the account's AI Gateway (BYOK unchanged; unset = direct endpoints); per-request caching/retries are constants in ai_proxy.js, default off. Keys are `npai_…` generated by `python3 tools/ai_key.py <name> [monthly_limit]` — only the SHA-256 hash is stored in `api_keys`. Monthly request-count breaker lives in `ai_usage` (atomic conditional upsert, 429 when exhausted, fail-open on D1 trouble) which also accumulates token totals; per-call metadata (never prompt/response content) goes to `ai_logs` — for streamed calls the log is written by the TransformStream pump BEFORE the client stream closes, because waitUntil D1 writes issued after a streamed response silently never land (see ai_proxy.js pumpStream). CORS is `*` for `/api/ai` (bearer auth, no cookies). A provider whose secret is unset returns 503. A daily Cron trigger (03:17 UTC) prunes `ai_logs` (>90d), `ai_usage` (>13mo) and `comment_rate` (>1d). New comments fire a fire-and-forget owner email via the `NOTIFY` send_email binding when it is configured in wrangler.jsonc (currently not configured — the code is dormant without it).
