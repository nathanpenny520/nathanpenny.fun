...
`workers/comments.js` (with imported modules `workers/access.js`, `workers/admin_page.js`, `workers/editor_page.js`, `workers/stats_page.js`, `workers/editor.js`, `workers/analytics.js` and `workers/ai_proxy.js`) is a Cloudflare Worker module using a D1 binding `env.DB` and an R2 binding `env.R2` (bucket `nathanpenny-fun`). Endpoints (full details in `workers/README.md`):
...
| GET    | `/admin`                       | Cloudflare Access | Admin page with four tabs: Images (file explorer) + Editor (写作台) + AI playground + Stats (analytics dashboard); UI is English-only |
